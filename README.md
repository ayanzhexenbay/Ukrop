...
Один GET делает все режимы:

1. **1-е число** → месячный список **текущего** месяца → группа + контакты  
2. **25-е число** → месячный список **следующего** месяца (заранее) → группа + контакты  
3. **Каждый день** → кто сегодня именинник → только контакты  
...
```
…&preview=1&date=25.08.2026     # ежедневное + аванс за сентябрь
…&preview=1&date=01.09.2026     # месячный список за сентябрь
…&force_monthly=1&preview=1
```

Отправка на один номер (тест):

```
# Ежедневное — кто именинник в этот день
…&date=29.08.2026&send=77778090088

# Месячный список за месяц
…&month=08.2026&send=77778090088

# Превью без отправки
…&preview=1&month=08.2026&send=77778090088
```

`send` — только указанный номер (без группы). Несколько: `send=777,77771835265`.
